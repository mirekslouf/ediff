...
class CrystParams:
    '''
    Define crystal = atom positions (structure) + temperature factors.
    
    Parameters
    ----------
    structure : structure object
        * The *structure object* contains positions of atoms in the crystal.
    ...
    def __init__(self, structure, temp_factors=0.8):
        ...
        * The parameters are described above in class definition.
        ...
        if type(structure) == pmStructure:
            ...
            # => already in correct format, just save it to self.structure
            self.structure = structure
        else:
            # Structure is not from PyMatGen => we suppose it is a CIF file
            # => read the CIF file using a PyMatGen method
            self.structure = pmStructure.from_file(structure)
        ...
        if type(temp_factors) == dict:
            ...
            # (if nothing was given, we use default = temp_factor = B = 0.8
            self.temp_factors = self._get_elements_with_temp_factors(
                self.structure, temp_factors)


    def _get_elements(self, structure):
        '''
    ...
    def _get_elements_with_temp_factors(self, structure, B=0.8):
        '''
...
class PXRDcalculation:
    ...
    * Even simpler API: see ediff.pcryst.XRD_polycrystal class.
    ...
    def __init__(self, cryst_params, simu_params):
        '''
        Initialize PXRDcalculatin object.
        
        Parameters
        ----------
        cryst_params : ediff.pcryst.CrystParams object
            This object is usually prepared in advance
            as an instance of ediff.pcryst.CrystParams class.
        simu_params : ediff.pcryst.Simu_params object
            This object is usually prepared in advance
        ...
        >>> 
        >>> # [1] Crystal and simulation parameters
        >>> # are defined as objects XTAL and SPAR, respectively.
        >>> XTAL = pcryst.CrystParams(structure=CIF_FILE, temp_factors=0.8)
        ...
        >>> CALC = pcryst.PXRDcalculation(XTAL,SPAR)
        >>> 
        ...
        >>> CALC.diffractogram.show('q','I','nacl_pxrd.py.png')
        >>> CALC.diffractogram.save(nacl_pxrd.py.txt')
        '''
        
        # (1) Read input parameters
        self.cryst_params = cryst_params
        self.simu_params  = simu_params
                
        # (2) Calculate diffractions
        # (PyMatGen calculation = call adjusted external function
        self.diffractions = self._calculate_diffractions()
        
        # (3) Calculate whole diffraction profile
        # (our own calculation = diffractions convoluted with peak_profile
        # (peak_profile parameters are defined within sumu_parameters object
    ...
    def _calculate_diffractions(self):
        ...
        calculation = pmXRDCalculator(
            wavelength = self.simu_params.wavelength,
            debye_waller_factors=(self.cryst_params.temp_factors))
        diffractions = calculation.get_pattern(
            self.cryst_params.structure,
            two_theta_range = self.simu_params.two_theta_range)
        
        # (2) Transform diffractions
    ...
    def _calculate_diffractogram(self):
        ...
        th1,th2 = th1,th2 = self.simu_params.two_theta_range
        ...
        if self.simu_params.peak_profile == None:
        ...
        peak_profile = self.simu_params.peak_profile(
            X, 0, self.simu_params.peak_profile_sigma)    
    ...
    def _diffractions_to_dframe(intensities):
        ...
        # * In each step, we have to differentiate
        #   non-hexagonal (hkl) vs. hexagonal indexing (hkil).
        # -----
        
        # (0) Test if the structure is hexagonal
        hexagonal = (len(intensities.hkls[0][0]['hkl']) == 4)
        
        # (1) Prepare hkl indexes
        if not(hexagonal):
            # non-hexagonals => three indices h,k,l
            h,k,l = [],[],[]
        else:
            # hexagonal systems => four indices h,k,i,l
            h,k,i,l = [],[],[],[]
        
        # (2) Fill in hkl (or hkil) indexes
        for ints in intensities.hkls:
            # all systems => [h,l] in columns [0,1]
            h.append(ints[0]['hkl'][0])
            k.append(ints[0]['hkl'][1])
            # non-hexagonals => [l] in column [2]
            if not(hexagonal):
                l.append(ints[0]['hkl'][2])
            # hexagonal systems => [i,l] in column [2,3] 
            else:    
                i.append(ints[0]['hkl'][2])
                l.append(ints[0]['hkl'][3])
        
        # (3) Create DataFrame from all values
        if not(hexagonal):
            # non-hexagonals => THREE indices h,k,l + dhkl,Ihkl
            df = pd.DataFrame( 
                np.transpose(
                    [intensities.x,
                     h, k, l, intensities.d_hkls, intensities.y]),
                columns=['TwoTheta','h','k','l','dhkl','Ihkl'])
            # format integer columns (h,k,l)
            # (this is important for nice printing
            df = df.astype({'h':'int32','k':'int32','l':'int32'})
        else:
            # hexagonal systems => FOUR indices h,k,i,l + dhkl,Ihkl
            df = pd.DataFrame( 
                np.transpose(
                    [intensities.x,
                     h, k, i, l, intensities.d_hkls, intensities.y]),
                columns=['TwoTheta','h','k','i', 'l','dhkl','Ihkl'])
            # format integer columns (h,k,i,l)
            # (this is important for nice printing
            df = df.astype({'h':'int32','k':'int32','i':'int32','l':'int32'})
                
        # (4) Calculate and insert columns with S,q
        if not(hexagonal):
            # Non-hexagonals => [S,q] = columns [5,6]
            df.insert(loc=5, column='S', value=1/df.dhkl)
            df.insert(loc=6, column='q', value=2*np.pi*df.S)
        else:
            # Hexagonal systems => [S,q] = columns [6,7]
            df.insert(loc=6, column='S', value=1/df.dhkl)
            df.insert(loc=7, column='q', value=2*np.pi*df.S)
        
        # Return the final dataframe
        # (the dataframe should be correct and prepared for nice printing
        # ( - it considers hexagonal indices
        # ( - it keeps all diffraction indices as integers
        return(df)
    ...
    def show_indexed_diffractions(self, out_file=None, two_theta=None):
        '''
        ...
        * The function is usually called without args to get interactive plot.
        ...
        if two_theta is None: two_theta = self.simu_params.two_theta_range
        # (c) create the plot using the parameters from (a,b) 
        xrd.get_plot(
            structure = self.cryst_params.structure,
    ...
    * Main advantage of XRD_polycrystal - simpler API than PXRDcalculation.
    ...
    def __init__(self, structure, temp_factors=0.8, description=None, 
        ...
        structure : CIF-file or ediff.io.Structure object
            ...
            More info about CIF-files is in ediff.pcryst.CrystParams class.
        ...
        temp_factors : float, optional, default is 0.8
            ...
            in the form of dictionary, as described in
            ediff.pcryst.CrystParams class.
        ...
        # (1) Obligatory ub-objects
        # Crystal parameters, Simulation parameters 
        cryst_params = CrystParams(
            structure, temp_factors)
        simu_params = SimuParams(
        ...
        PXRDcalculation.__init__(self, cryst_params, simu_params)
    ...
    * OO-processing (new, simplified interface, better for most cases).
    ...
    def find_center(self, **kwargs): 
        ...
        * icut : intensity cut; to cut too high intensity of primary beam
        ...
        # (1) Set final_print=False if not explicitly set by the user
    ...
    def subtract_background(self, method=None, **kwargs):
        ...
        * bground.api.Wavelets = ediff.bkg.Wavelets
        ...
        # (1) Default value of saveTXT parameter
        # (if it was not set explicitly by the user, adjust it
        if method == 'InteractivePlot': 
            # For interactive-plot-method,
            # we usually want to save BKG points to a file.
            kwargs.setdefault('saveTXT', True)
        else:
            # For non-interactive-plot methods, 
            # it is usually enough to save data in the calling object.
