...
class DiffData1D(pd.DataFrame):
    ...
    * The object has a few additional methods (has, show, save, pprint),
      some of which are adjusted in the above-mentioned subclasses.
    '''
    ...
    #   >>> diff1.name = 'Au powder'
    #   >>> diff2 = diff1[diff1.q > 1]
    #   >>> diff2.name  # works only if _metadata=['name'], otherwise error
    
    _metadata = ['name']
    
    # (2) DataFrame subclassing, item 2 :: _constructor
    # Necessary class property to keep our object after any pandas operation.
    # * pandas operations return DataFrame (if _constructor not defined)
    #   >>> type(diff1)        # DiffData1D
    #   >>> type(diff1.copy()  # pandas.DataFrame => wrong!
    ...
    def has(self, column):
        '''
        Test if the {DiffData1D} object contains specific column.
    ...
    def show(self, X, Y, 
             ...
             **kwargs):
        '''
        Show/plot {DiffData1D} in a simple and stadnard way.
    ...
    def save(self, out_file, pretty=True, fine_tune=None, **kwargs):
        '''
        Save {DiffData1D} object to TXT-file in EDIFF format.
    ...
    def pprint(self, index=False, float_format='{: 0.3f}'):
        '''
        Pretty print of {DiffData1D} object.

        Parameters
        ----------
        index : bool, optional, default is False
            If false, do not print index of DiffData1D/DataFrame object.
        float_format : string/format spec, optional, default is '{: 0.3f}'
            Format of float numbers in the DiffData1D/DataFrame object.

        Returns
        -------
        None
            The DataFrame containing XYdata is just nicely formatted
            and printed to stdout.
        '''
        # Adjust index (re-define index so that it started from 1 instead of 0
        if index is True: df = self.copy(); df.index = range(1,len(df)+1)
        # Convert dataframe to string/table
        table = df.to_string(index=index, float_format=float_format.format)
        # Print the nicely formated dataframe to stdout
        print(table)
...
class Diffractions1D(DiffData1D):
    ...
    def show(self, X, Y, 
             ...
             title=None, out_file=None, out_dpi=300,
             **kwargs):
        # Just call the Data1D.show with 4th parameter = 'vlines'
        # => this function is just a simple wrapper around Data1D.show
        Data1D.show(self, X, Y, 'vlines',
                    Xlabel, Ylabel, Xlim, Ylim,
                    title, out_file, out_dpi, **kwargs)
    
    
    def pprint(self, index=True, float_format='{: 0.3f}'):
        # Just call DiffData1D.pprint with above-adjusted default arguments
        super().pprint(index, float_format)
...
class Diffractogram1D(DiffData1D):
    ...
    def show(self, X, Y, 
             ...
             title=None, out_file=None, out_dpi=300,
             **kwargs):
        # Just call the Data1D.show with 4th parameter = 'plot'
        # => this function is just a simple wrapper around Data1D.show
    ...
    def pprint(index=False, float_format='{: 0.3f}'):
        # Just call DiffData1D.pprint with above-adjusted default arguments
        super().pprint(index, float_format)
...
class Plots:
    ...
    * _saved_rcParams    = employed in restore_saved_params
    ...
    @staticmethod    
    def plot_multiple_eld_and_xrd(eld_profile, xrd_profile, fine_tune, Xlim,
            eld_data_label='ED experiment', xrd_data_label='XRD calculation',
            Xlabel='$q$ [1/\u212B]', Ylabel='Intensity',
            Xticks=None, Yticks=None, mXticks=None, mYticks=None,
            out_file=None, out_dpi=300, transparent=False, CLI=False):
        '''
        Comparison of multiple ELD and with final XRD profiles.
    
        * FUNCTION JUST FOR TESTING, extension of {plot_final_eld_and_xrd}.
            - Why just for testing? => final comparison = one ELD x one XRD.
            - Why it is kept here? => it is used in Jakub's testing scripts.
        * During the final plotting, we fine-tune the ELD calibration(s).
        * This is done by iterative modification of fine_tuning constant(s).
        
        Parameters
        ----------
        eld_profile : str, numpy.array, or list/iterable of them
            The *eld_profile* (ELD) is
            an electron diffraction profile in EDIFF format.
            It can come as file (if *eld_profile* = str = filename)
            or array (if *eld_profile* = numpy.array).
            Can also be passed as a list/iterable of multiple profiles.
        ...
        xrd_profile : str or numpy.array
            ...
            => see docs of ediff.calibration.Calculate.from_max_peaks function.
        fine_tune : float or list of floats
            The constant for the final fine-tuning of peak position.
            The *fine_tuning* constant has a starting value of 1.000.
            If ELD and XRD peaks are shifted, the constant should be adjusted.
            The constant multiplies the X-values of ELD profile.
            Can be passed as a single float or a list matching the number of ELD profiles.
        Xlim : tuple of two floats
            The limits for X-axis (minimum and maximu q-vectors on X-axis).
        eld_data_label : str or list of str, optional, default is 'ED experiment'
            The label of ELD data (= name of the electron diffraction data).
            Can be passed as a single string or a list matching the number of ELD profiles.
        xrd_data_label : str, optional, the default is 'XRD calculation'
            The label of XRD data (= name of the X-ray diffraction data).
        Xlabel : str, optional, the default is '$q$ [1/\u212B]' ~ q [1/A]
            The label of X-axis.
        ...
        # Normalize eld_profile input into a list
        if not isinstance(eld_profile, (list, tuple)):
            eld_list = [eld_profile]
        else:
            eld_list = list(eld_profile)
            
        num_eld = len(eld_list)
        
        # Normalize fine_tune input into a list matching the number of ELD profiles
        if isinstance(fine_tune, (int, float)):
            fine_tune_list = [fine_tune] * num_eld
        else:
            fine_tune_list = list(fine_tune)
            if len(fine_tune_list) != num_eld:
                raise ValueError("Length of 'fine_tune' list must match the number of ELD profiles.")
                
        # Normalize eld_data_label input into a list matching the number of ELD profiles
        if isinstance(eld_data_label, str):
            if num_eld == 1:
                label_list = [eld_data_label]
            else:
                label_list = [f"{eld_data_label} {i+1}" for i in range(num_eld)]
        else:
            label_list = list(eld_data_label)
            if len(label_list) != num_eld:
                raise ValueError("Length of 'eld_data_label' list must match the number of ELD profiles.")

        # Plot all ELD profiles
        for i, eld in enumerate(eld_list):
            ft = fine_tune_list[i]
            lbl = label_list[i]
            
            # Preserve original red color behavior if only one profile is present
            if num_eld == 1:
                plt.plot(eld.q * ft, eld.I, color='red', label=lbl)
            else:
                plt.plot(eld.q * ft, eld.I, label=lbl)
        ...
        if mYticks is not None:
            plt.gca().yaxis.set_minor_locator(plt.MultipleLocator(mYticks))
        # Add legend (considering transparency)
        if transparent == True:
            plt.legend(framealpha=0.0)
        else: plt.legend()
